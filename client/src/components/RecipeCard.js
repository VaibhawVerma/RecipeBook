...
const RecipeCard = ({ recipe }) => {
    ...
    const handleAuthorClick = (e) => {
        // This prevents the click from navigating to the recipe detail page
    ...
    return (
        <Link to={`/recipe/${recipe._id}`} className="block group">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 group-hover:shadow-2xl group-hover:-translate-y-1 h-full flex flex-col">
                <img src={recipe.imageUrl || placeholderImage} alt={recipe.title} className="w-full h-48 object-cover" onError={(e) => { e.target.onerror = null; e.target.src=placeholderImage; }} />
