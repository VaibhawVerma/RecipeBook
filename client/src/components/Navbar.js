...
const Navbar = ({ setIsAuthenticated }) => {
    ...
    return (
        <header className="bg-white/90 backdrop-blur-lg shadow-sm sticky top-0 z-50">
            ...
            {isOpen && (<div className="md:hidden bg-white px-4 pt-2 pb-4 space-y-2 border-t">{navLinks}<button onClick={onLogout} className="w-full text-left text-red-600 py-2">Logout</button></div>)}
        </header>
    );
};
