...
const CreateRecipe = () => {
    ...
    useEffect(() => { if (id) { const fetchRecipe = async () => { try { const { data } = await axios.get(`/api/recipes/${id}`); setFormData({ title: data.title, description: data.description, ingredients: data.ingredients.join(', '), instructions: data.instructions, category: data.category }); if (data.imageUrl) { setImagePreview(data.imageUrl); } } catch (err) { setError("Could not load recipe data."); } }; fetchRecipe(); } }, [id]);
    ...
    const onFileChange = async (e) => {
        ...
        if (file.size / 1024 / 1024 > maxSizeMB) {
            setError(`File is too large. Please upload an image smaller than ${maxSizeMB}MB.`);
            return;
        }
        setError('');
        ...
        try {
            ...
            showToast('Could not compress image, using original file.', 'error');
            setImageFile(file); // Fallback to the original file
        }
    };
    ...
    const onSubmit = async (e) => {
        ...
        // Use the imageFile state which holds either the compressed or original file
        if (imageFile) {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.msg || 'An error occurred while saving';
                        ...
                        <div>
                            ...
                            {imagePreview ? (
                                <div className="relative group">
                                    <img src={imagePreview} alt="Recipe preview" className="w-full h-64 object-cover rounded-lg shadow-inner" />
                                    <button type="button" onClick={removeImage} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity" title="Remove image">
                                    ...
                                    <div className="space-y-1 text-center">
                                        ...
                                        <div className="flex text-sm text-gray-600">
                                            <label htmlFor="file-upload" className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500">
                                                ...
                                                <input id="file-upload" name="image" type="file" accept="image/png, image/jpeg, image/jpg" onChange={onFileChange} className="sr-only" />
                                            </label>
                                        </div>
