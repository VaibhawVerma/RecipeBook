...
const Home = () => {
    ...
    // 3. The main data fetching now depends on the searchParams from the URL.
    useEffect(() => {
        const fetchRecipes = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`/api/recipes?${searchParams.toString()}`);
    ...
    // Effect for fetching autocomplete suggestions (no changes here)
    ...
    // 4. All state changes are now functions that update the URL search parameters.
    ...
    return (
        <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div className="relative">
                    ...
                    {suggestions.length > 0 && (
                        <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg mt-1 shadow-lg">
                            {suggestions.map(suggestion => ( <li key={suggestion._id} onClick={() => handleSuggestionClick(suggestion.title)} className="px-4 py-2 cursor-pointer hover:bg-gray-100">{suggestion.title}</li> ))}
                        </ul>
                    )}
                </div>
            </div>
            ...
            {loading ? ( <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8"> {Array.from({ length: 8 }).map((_, index) => ( <RecipeCardSkeleton key={index} /> ))} </div> ) : recipes.length === 0 ? ( <div className="text-center py-10 bg-white rounded-lg shadow-sm"> <h3 className="text-xl font-medium text-gray-800">No Recipes Found</h3> <p className="text-gray-500 mt-2">{searchTerm || activeCategory ? `We couldn't find any recipes for your criteria.` : "No recipes have been shared yet."}</p> </div> ) : ( <> <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8"> {recipes.map(recipe => <RecipeCard key={recipe._id} recipe={recipe} />)} </div> <Pagination page={page} pages={pages} onPageChange={handlePageChange} /> </> )}
        </div>
    );
};
